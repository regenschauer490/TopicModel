...
#include "../model/common/lda_interface.hpp"
...
struct MapValue
{
	...
	MapValue() = default;
	//MapValue(uint num_topics, uint word_num) : knum_(num_topics), vnum_(word_num){}
	MapValue(uint max_local_iteration, uint num_topics, uint word_num, VectorK<double> const& alpha, MatrixKV<double> const& phi, VectorV<uint> const& word_ct, VectorK<double>& gamma)
		: knum_(num_topics), vnum_(word_num), word_ct_(&word_ct), alpha_(&alpha), phi_(&phi), gamma_(&gamma){}
};
