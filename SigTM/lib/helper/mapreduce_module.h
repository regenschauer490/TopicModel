...
struct MapValue
{
	...
	//MatrixKV<double> const* lambda_;
	MatrixKV<double> const* beta_;
	VectorK<double>* gamma_;		// gammaは各mapperで更新

	MapValue() = default;
	MapValue(uint topic_num, uint word_num) : knum_(topic_num), vnum_(word_num){}
	MapValue(uint topic_num, uint word_num, VectorK<double> const& alpha, /*MatrixKV<double> const& lambda*/ MatrixKV<double> const& beta, VectorV<uint> const& word_ct, VectorK<double>& gamma)
		: knum_(topic_num), vnum_(word_num), word_ct_(&word_ct), alpha_(&alpha), /*lambda_(&lambda)*/ beta_(&beta), gamma_(&gamma){}
};
